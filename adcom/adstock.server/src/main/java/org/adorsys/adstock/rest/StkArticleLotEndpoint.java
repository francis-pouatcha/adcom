...
public class StkArticleLotEndpoint {
	...
	public StkArticleLotSearchResult findBy(StkArticleLotSearchInput searchInput) {
		...
		StkArticleLotSearchResult searchResult = new StkArticleLotSearchResult(
				count, detach(resultList), detach(searchInput));
	...
	private StkArticleLotSearchResult findArticleLotProducts(
		...
		for (String artPic : artPics) {
			...
			List<StkArticleLot> resultList = ejb.findBy(
	...
	@Produces({ "application/json", "application/xml" })
	@Consumes({ "application/json", "application/xml" })
	public StkArticleLotSearchResult findByLike(
			StkArticleLotSearchInput searchInput) {
		SingularAttribute<StkArticleLot, ?>[] attributes = readSeachAttributes(searchInput);
		Long countLike = ejb.countByLike(searchInput.getEntity(), attributes);
		List<StkArticleLot> resultList = ejb.findByLike(
				searchInput.getEntity(), searchInput.getStart(),
				searchInput.getMax(), attributes);
		StkArticleLotSearchResult searchResult = new StkArticleLotSearchResult(
				countLike, detach(resultList), detach(searchInput));
		return processSearchResult(searchInput, searchResult);
	}
	...
	@Produces({ "application/json", "application/xml" })
	@Consumes({ "application/json", "application/xml" })
	public ArticleLotSearchResult findArticleLotByProductscip(
	...
	public StkArticleLotSearchResult findCustom(
		...
		List<StkArticleLot> resultList = ejb.findCustom(searchInput);
		...
		for (String fieldName : fieldNames) {
			...
			for (Field field : fields) {
				if (field.getName().equals(fieldName)) {
					try {
						result.add((SingularAttribute<StkArticleLot, ?>) field
								.get(null));
