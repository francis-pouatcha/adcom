	...
	public PrcmtOrderHolder executeTriggerMode(PrcmtProcOrder prcmtProcOrder) {	
		...
		for(StkLotStockQty itemStock:itemsShortage){
			...
			if(!findByArtPic.isEmpty()){
				...
				prcmtPOItem.setStrgSection(itemStock.getSection());
				prcmtPOItem.setPppuCur(sectionArticleLot.getPppuCur());
				prcmtPOItem.setPppuPreTax(sectionArticleLot.getPppuHT());
				prcmtPOItem.setVatPct(sectionArticleLot.getVatPurchPct());
