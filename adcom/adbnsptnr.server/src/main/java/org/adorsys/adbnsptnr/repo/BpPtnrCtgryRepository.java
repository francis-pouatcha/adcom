...
{
	@Query("SELECT e FROM BpPtnrCtgry AS e WHERE e.identif = ?1 AND e.validFrom <= ?2 AND (e.validTo IS NULL OR e.validTo > ?2)")
	public QueryResult<BpPtnrCtgry> findByIdentif(String identif, Date validOn);
}
